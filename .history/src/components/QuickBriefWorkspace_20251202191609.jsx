...
const initialSlides = [
    ...
    // Slide 8: Strategic Recommendations - Part 1
    {
        ...
        elements: [
            ...
            {
                id: 'rec-heroes', type: 'strategic-rec-card', colSpan: 3, priority: 'HIGH', icon: 'trending',
            ...
            {
                id: 'rec-mold', type: 'strategic-rec-card', colSpan: 3, priority: 'CRITICAL', icon: 'alert',
            ...
            {
                id: 'rec-durability', type: 'strategic-rec-card', colSpan: 3, priority: 'HIGH', icon: 'wrench',
    ...
    // Slide 9: Strategic Recommendations - Part 2
    {
        ...
        elements: [
            ...
            {
                id: 'rec-premium', type: 'strategic-rec-card', colSpan: 3, priority: 'MEDIUM', icon: 'target',
            ...
            {
                id: 'rec-app', type: 'strategic-rec-card', colSpan: 3, priority: 'MEDIUM', icon: 'smartphone',
                title: 'Reposition App as Value Amplifier',
                rationale: 'Users see app as optional. Making value tangible creates differentiation',
                actions: ['Simplify onboarding with "1 minute win" experience', 'Add quick benefits: better coverage, dentist tracking', 'Use app data to trigger coaching before engagement drops'],
                impact: 'Increase perceived value of smart features'
            },
            {
                id: 'rec-pricing', type: 'strategic-rec-card', colSpan: 3, priority: 'MEDIUM', icon: 'dollar',
                title: 'Refine Pricing & Promo Strategy',
                rationale: 'Consumers respond positively to event discounts',
                actions: ['Strategic event-based discounts (Black Friday)', 'Create value packs for iO 3/5 bundling replacement heads', 'Consider loyalty program for repeat head purchases'],
                impact: 'Optimize price perception and lifetime value'
            },
            {
                id: 'rec-language', type: 'strategic-rec-card', colSpan: 3, priority: 'HIGH', icon: 'message',
    ...
    // Slide 10: Strategic Summary & Next Steps
    {
        ...
        elements: [
            ...
            {
                id: 'monitoring', type: 'strategic-rec-card', colSpan: 3, priority: 'ONGOING', icon: 'chart',
            ...
            {
                id: 'what-so-now', type: 'what-so-now-grid', colSpan: 3, title: 'What, So What, Now What - Summary',
                columns: [
            ...
            {
                id: 'next-steps', type: 'next-steps-grid', colSpan: 3, title: 'Recommended Next Steps',
                steps: [
    ...
    return (
        ...
        >
            <div className="absolute top-2 right-2 flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity z-10">
                ...
                <button className="p-1 hover:bg-slate-100 rounded text-slate-400 hover:text-slate-600 bg-white shadow-sm border border-slate-200">
                    <Pencil size={12} />
                </button>
            ...
            {element.type === 'strategic-rec-card' && (
                <div className={`rounded-xl p-5 border-l-4 border ${
                    element.priority === 'CRITICAL' ? 'border-l-red-500 bg-gradient-to-r from-red-50 to-white border-slate-200' :
                    element.priority === 'HIGH' ? 'border-l-orange-400 bg-gradient-to-r from-orange-50 to-white border-slate-200' :
                    element.priority === 'MEDIUM' ? 'border-l-amber-300 bg-gradient-to-r from-amber-50 to-white border-slate-200' :
                    element.priority === 'ONGOING' ? 'border-l-indigo-400 bg-gradient-to-r from-indigo-50 to-white border-slate-200' :
                    'border-l-slate-300 bg-white border-slate-200'
                }`}>
                    <div className="flex items-start justify-between mb-3">
                        <div className="flex items-center gap-3">
                            <div className={`w-10 h-10 rounded-lg flex items-center justify-center ${
                                element.priority === 'CRITICAL' ? 'bg-red-100' :
                                element.priority === 'HIGH' ? 'bg-orange-100' :
                                element.priority === 'MEDIUM' ? 'bg-amber-100' :
                                element.priority === 'ONGOING' ? 'bg-indigo-100' :
                                'bg-slate-100'
                            }`}>
                                {element.icon === 'trending' && <TrendingUp className={`w-5 h-5 ${element.priority === 'CRITICAL' ? 'text-red-600' : element.priority === 'HIGH' ? 'text-orange-600' : 'text-amber-600'}`} />}
                                {element.icon === 'alert' && <AlertTriangle className="w-5 h-5 text-red-600" />}
                                {element.icon === 'wrench' && <Wrench className={`w-5 h-5 ${element.priority === 'CRITICAL' ? 'text-red-600' : 'text-orange-600'}`} />}
                                {element.icon === 'info' && <Info className="w-5 h-5 text-slate-600" />}
                                {element.icon === 'target' && <Target className="w-5 h-5 text-amber-600" />}
                                {element.icon === 'smartphone' && <Smartphone className="w-5 h-5 text-amber-600" />}
                                {element.icon === 'dollar' && <DollarSign className="w-5 h-5 text-amber-600" />}
                                {element.icon === 'message' && <MessageCircle className="w-5 h-5 text-orange-600" />}
                                {element.icon === 'chart' && <BarChart2 className="w-5 h-5 text-indigo-600" />}
                            </div>
                            <h3 className="text-slate-900 font-semibold">{element.title}</h3>
                        </div>
                        <span className={`px-3 py-1 rounded-full text-xs font-medium ${
                            element.priority === 'CRITICAL' ? 'bg-red-500 text-white' :
                            element.priority === 'HIGH' ? 'bg-orange-400 text-white' :
                            element.priority === 'MEDIUM' ? 'bg-amber-300 text-amber-800' :
                            element.priority === 'ONGOING' ? 'bg-indigo-100 text-indigo-700 border border-indigo-200' :
                    ...
                    {element.rationale && (
                        <p className="text-slate-600 text-sm mb-4">{element.rationale}</p>
                    )}
                    
                    {element.actions && (
                        <div className="mb-4">
                            <p className="text-slate-700 text-sm font-medium mb-2">Actions:</p>
                    ...
                    {element.impact && (
                        <div className="bg-slate-50 rounded-lg p-3 border border-slate-100">
                            <p className="text-slate-600 text-sm">
                                <span className="font-medium text-slate-700">Impact:</span> {element.impact}
                            </p>
                        </div>
                    )}
                </div>
                    ...
                    <div className="grid grid-cols-3 gap-4">
                        {element.columns?.map((col, i) => (
                            <div key={i} className="bg-slate-50 rounded-lg p-4 border border-slate-200">
                                <h4 className="text-slate-900 font-semibold text-sm mb-3 uppercase tracking-wide">{col.title}</h4>
            ...
            {element.type === 'next-steps-grid' && (
                <div className="bg-gradient-to-r from-orange-50 to-amber-50 rounded-xl p-5 border-l-4 border-l-orange-400 border border-slate-200">
                    {element.title && (
                        <h3 className="text-slate-900 font-medium mb-4">{element.title}</h3>
                    )}
                    <div className="grid grid-cols-2 gap-3">
...
const SlideBlock = ({ slide, index, onUpdate, onAddElement, onDeleteElement, onUpdateElement, onRun, isActive, onReorder, dragHandleProps }) => {
        ...
        <div className="w-full bg-white group/slide relative">
            ...
            <div className="mb-6">
                <textarea
                    ...
                    className="w-full text-slate-500 bg-transparent border-none focus:outline-none focus:ring-0 resize-none text-sm placeholder-slate-300"
            ...
            <div className="flex-1 overflow-y-auto bg-white">
                <div className="max-w-5xl mx-auto px-12 py-12">
                    <div className="mb-12">
                        ...
                        <p className="text-slate-500 text-lg">Interactive workspace for Oral-B iO customer satisfaction..</p>
                    </div>
                    ...
                    <div className="py-12 border-t border-dashed border-slate-200 mt-12 text-center">
                        <button
                            onClick={addSlide}
                            className="inline-flex items-center gap-2 px-6 py-3 bg-slate-900 text-white rounded-full hover:bg-slate-800 transition-all font-medium"
                        >
                            <Plus size={20} />
