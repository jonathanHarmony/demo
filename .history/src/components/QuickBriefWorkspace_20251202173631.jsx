...
const SlideBlock = ({ slide, onUpdate, onAddElement, onDeleteElement, onUpdateElement, onRun, isActive }) => {
    ...
    return (
        <div className="w-full bg-white mb-12 group/slide">
            ...
            {/* Tabs Navigation */}
            <div className="flex items-center gap-1 mb-6 bg-slate-50 p-1 rounded-lg w-fit border border-slate-200">
                {tabs.map(tab => (
                    <button
                        key={tab.id}
                    ...
                    <div className="relative bg-slate-50/50 border border-slate-100 rounded-xl p-6 min-h-[400px]">
                        ...
                        <div className="grid grid-cols-2 gap-6 auto-rows-min">
                            {slide.elements.map((element) => (
                                <div key={element.id} className={`${element.type === 'heading' || element.type === 'text' ? 'col-span-2' : 'col-span-1 min-h-[300px]'}`}>
                                    <CanvasElement
                                        element={element}
