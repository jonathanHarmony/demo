        ...
        elements: [
            { id: 'feat-intro', type: 'text', colSpan: 3, content: 'Detailed analysis of key product features and user feedback' },
            {
                ...
                features: [
                    ...
                    { icon: 'alert', title: 'Brush Head Design', mentions: 270, status: 'CRITICAL', statusColor: 'red', description: 'Mold formation concerns - critical hygiene issue.', action: 'URGENT: Audit design & communicate fixes' },
            ...
            {element.type === 'feature-cards-grid' && (
                <div className="grid grid-cols-2 gap-4">
                    {element.features?.map((feat, i) => {
                        ...
                        return (
                            <div key={i} className={`bg-white rounded-xl p-4 border border-slate-200 border-l-4 ${getBorderColor(feat.statusColor)}`}>
                                <div className="flex items-start justify-between mb-3">
                                    <div className="flex items-center gap-3">
                                        <div className={`w-10 h-10 rounded-lg flex items-center justify-center ${getIconBg(feat.statusColor)}`}>
                                            {getFeatureIcon(feat.icon)}
                                        </div>
                                        <div>
                                            <h4 className="text-slate-900 font-medium text-sm">{feat.title}</h4>
                                            <p className="text-slate-400 text-xs">{feat.mentions} mentions</p>
                                        </div>
                                    </div>
                                    <span className={`px-2 py-0.5 rounded text-xs font-medium text-white ${getStatusBg(feat.statusColor)}`}>
                                        {feat.status}
                                    </span>
                                </div>
                                <p className="text-slate-600 text-sm mb-3">{feat.description}</p>
                                <div className="bg-slate-50 rounded-lg p-2">
                                    <p className="text-slate-500 text-xs"><span className="font-medium">Action:</span> {feat.action}</p>
                                </div>
                            </div>
                        );
                    })}
