  ...
  const renderSlide = () => {
    switch (slideIndex) {
      ...
      case 19:
        ...
        return (
          <div className="h-full p-12 flex flex-col">
            <h2 className="text-3xl font-bold mb-8 text-center text-harmonyDark">סיכום השוואתי: שתי קטגוריות שונות בתודעת הצרכן</h2>
            <div className="flex-grow flex justify-center">
              <div className="w-full max-w-4xl">
                <div className="grid grid-cols-4 font-bold text-lg mb-4 text-center">
                ...
                {compareData.map((row, i) => (
                  <div key={i} className={`grid grid-cols-4 items-center mb-2 p-3 rounded-lg ${i % 2 === 0 ? 'bg-gray-50' : 'bg-white'}`}>
                    {/* Left Bar Remilk */}
                    <div className="flex justify-end items-center gap-2">
                       <span className="text-sm font-bold text-harmonyDark">{row.r}</span>
                       <div className="h-2 bg-harmonyDark rounded-full" style={{width: `${Math.min(row.r / 2, 100)}px`}}></div>
                    </div>
                    
                    {/* Center Text */}
                    <div className="col-span-2 text-center font-medium text-gray-700">{row.theme}</div>

                    {/* Right Bar Cowfree */}
                    <div className="flex justify-start items-center gap-2">
                       <div className="h-2 bg-harmonyTurquoise rounded-full" style={{width: `${Math.min(row.c / 2, 100)}px`}}></div>
                       <span className="text-sm font-bold text-teal-600">{row.c}</span>
                    </div>
                  </div>
                ))}
              </div>
