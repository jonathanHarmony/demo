...
export default function ReportChat() {
    ...
    useEffect(() => {
        const loadHistory = async () => {
            try {
                const response = await fetch(`http://localhost:8000/chat/history/${SESSION_ID}`);
                if (response.ok) {
    ...
    const saveChatHistory = async (messagesToSave) => {
        try {
            await fetch('http://localhost:8000/chat/history/save', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                },
    ...
    return (
        <div className="flex flex-col h-full bg-white border-l border-slate-200" dir={isRTL ? 'rtl' : 'ltr'}>
            {/* Header */}
                ...
                <div className="space-y-6">
                                ...
                                <div className={`p-4 rounded-2xl text-sm leading-relaxed shadow-sm ${msg.role === 'user'
                                    ...
                                    <ReactMarkdown
                                        ...
                                        components={{
                                            ...
                                            ul: ({ node, ...props }) => <ul className="list-disc list-outside space-y-1 mb-2 pr-5" style={{ direction: 'rtl' }} {...props} />,
                    ...
                    {isTyping && (
                        <div className="flex gap-4 flex-row-reverse animate-in fade-in slide-in-from-bottom-2 duration-300">
                            ...
                            <div className="bg-gradient-to-br from-slate-50 to-slate-100 border border-slate-200 px-5 py-3 rounded-2xl rounded-tl-none shadow-sm">
                                <div className="flex items-center gap-3">
