...
const ResearchAssistant = forwardRef(({ sessionId = 'quickbrief' }, ref) => {
    ...
    return (
        <div className="w-80 flex flex-col h-full bg-white border-r border-slate-200">
            {/* Header */}
            ...
            <div className="p-4 bg-white border-t border-slate-100">
                ...
                {selectedTags.length > 0 && (
                    <div className="flex flex-wrap gap-1.5 mb-2">
                        {selectedTags.map((tag, idx) => (
                            <span 
                                key={idx}
                                className="inline-flex items-center gap-1 px-2 py-1 bg-slate-900 text-white text-xs rounded-md"
                            >
                                {tag}
                                <button 
                ...
                <div className="relative flex items-end gap-2">
                    <textarea
                        ...
                        placeholder={selectedTags.length > 0 ? "What would you like to do with this?" : "Ask about the analysis..."}
                        className="flex-1 min-h-[44px] max-h-[120px] rounded-xl border border-slate-200 focus:border-slate-400 focus:ring-0 focus:outline-none bg-slate-50 px-4 py-3 resize-none text-sm"
                        rows={1}
                    />
                    <button
                        onClick={handleSend}
